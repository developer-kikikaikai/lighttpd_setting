...
static void mod_loopcgi_request_info_free(mod_loopcgi_request_info_t * req_info, server *srv);
/* @} */
...
static void mod_loopcgi_con_info_free(LOOPCGI_CON_INFO this, server *srv) {
	fdevent_event_del(srv->ev, &this->fdevent_id, this->timerfd);
	fdevent_unregister(srv->ev, this->timerfd);
...
ENTERLOG
	...
	timer.it_interval.tv_sec = this->setting.interval;
	timer.it_value.tv_sec = this->setting.interval;
...
static void mod_loopcgi_request_info_free(mod_loopcgi_request_info_t * req_info, server *srv) {
	...
	while(info) {
		mod_loopcgi_con_info_free(info, srv);
...
ENTERLOG
	...
	//free request info
	mod_loopcgi_request_info_free(&p->request_info, srv);
	free(p);
...
ENTERLOG
	...
	//already requested it. Called from fdevent trigger
	LOOPCGI_CON_INFO con_info = mod_loopcgi_get_coninfo(con, &p->request_info);
	if(con_info) return HANDLER_FINISHED;
	...
	LOOPCGI_CON_INFO instance = mod_loopcgi_con_info_new(srv, con, &setting);
...
int mod_loopcgi_fd_plugin_init(plugin *p);
int mod_loopcgi_fd_plugin_init(plugin *p) {
	p->version     = LIGHTTPD_VERSION_ID;
	p->name        = buffer_init_string("loopcgi_fd");
